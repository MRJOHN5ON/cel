#!/usr/bin/env python3
"""Cel macOS app launcher — local server + native app window."""
...
def pick_port() -> int:
    preferred = int(os.environ.get("CEL_PORT", "8742") or "8742")
    for port in (preferred, 0):
        try:
            ...
            continue
    raise SystemExit("Could not bind a local port for Cel")
...
def configure_environment(root: Path) -> int:
    ...
    # Allow imports from Resources (macos_about, etc.)
    sys.path.insert(0, str(root))

    return pick_port()


def apply_cel_bundle_context() -> None:
    """Rename the process/menu without replacing NSBundle.mainBundle.

    Replacing mainBundle breaks WKWebView initialization and causes a blank window.
    """
...
def schedule_menu_rename(name: str = APP_NAME) -> None:
    """Menu updates must run on the Cocoa main thread."""
    if sys.platform != "darwin":
        return
    try:
...
def wait_for_server(port: int, timeout: float = 45.0) -> None:
    ...
    raise SystemExit(f"Cel UI did not become ready on port {port}")
...
def run_server(port: int) -> None:
    ...
    try:
        ...
        print(f"Cel server crashed: {exc}", flush=True)
        raise


def get_window_geometry() -> tuple[int, int, int | None, int | None]:
    """Size and position the window to the main screen's visible frame."""
    if sys.platform != "darwin":
...
def main() -> None:
    ...
    print(f"Cel starting at {url}", flush=True)
    ...
    def on_shown() -> None:
        ...
        print(f"Cel window shown — {window.get_current_url()}", flush=True)

    def on_loaded() -> None:
        apply_cel_bundle_context()
        schedule_menu_rename(APP_NAME)
        print(f"Cel UI loaded — {window.get_current_url()}", flush=True)
